...
from lucid.modelzoo.aligned_activations import (
    get_aligned_activations as _get_aligned_activations,
)
from lucid.misc.io import load, save
import lucid.misc.io.showing as showing

from lucid.modelzoo.metadata import graph_metadata
from lucid.modelzoo.deferred_tensor import DeferredTensor, TensorCollection
...
class Layer(object):
    ...
    def __repr__(self):
        return "Layer (belonging to {s.model_class.name}) <{s.name}: {s.depth}> ([{s.tags}])".format(
            s=self
        )
...
class Model(with_metaclass(ModelPropertiesMetaClass, object)):
    ...
    def __init__(self):
        meta = graph_metadata(self)

        self.tensors = TensorCollection()
        self.tensors_to_visualize = []
        op_names = [key for key in meta["ops"].keys() if key != "input:0"]
        prefix = path.commonpath(op_names)
        for identifier, op in meta["ops"].items():
            ...
            rank = op.get("rank")
            dt = DeferredTensor(
                self, identifier, channels, shape, op_type, rank, ignored_prefix=prefix
            )
            self.tensors.add(dt)
            if op_type in ("Relu", "Conv2D", "Concat", "ConcatV2") and rank == 4:
                self.tensors_to_visualize.append(dt)
    ...
    def suggest_save_args(graph_def=None):
        ...
        if len(potential_input_nodes) == 1:
            ...
            if input_dtype.is_floating:
                ...
                print(
                    "Inferred: input_name = {} (because it was the only Placeholder in the graph_def)".format(
                        input_name
                    )
                )
                ...
                if divs and magic_number_present:
                    if len(divs) == 1:
                        ...
                        print(
                            "Guessed: input_name = {} (because it's the only division by 255 near the only placeholder)".format(
                                input_name
                            )
                        )
                        inferred_info["input_name"] = input_name
                        ...
                        print(
                            "Guessed: image_value_range = {} (because you're dividing by 255 near the only placeholder)".format(
                                image_value_range
                            )
                        )
                        ...
                        warnings.warn(
                            "Could not infer input_name because there were multiple division ops near your the only placeholder. Candidates include: {}".format(
                                [n.name for n in divs]
                            )
                        )
        else:
            warnings.warn(
        ...
        if output_nodes:
            print(
                "Inferred: output_names = {}  (because those are all the Softmax ops)".format(
                    output_nodes
                )
            )
    ...
    def load(graphdef_url):
        ...
        if metadata:
            ...
            raise ValueError(
                "Model.load was called on a GraphDef ({}) that does not contain Lucid's metadata node. Model.load only works for models saved via Model.save. For the graphdef you're trying to load, you will need to provide custom metadata; see Model.load_from_metadata()".format(
                    graphdef_url
                )
            )
    ...
    def load_from_manifest(manifest_url):
        try:
            ...
            raise ValueError(
                "Could not find manifest.json file in dir {}. Error: {}".format(
                    manifest_url, e
                )
            )
        ...
        if manifest.get("type", "frozen") == "frozen":
            ...
            raise NotImplementedError(
                "SerializedModel Manifest type '{}' has not been implemented!".format(
                    manifest.get("type")
                )
            )
...
class SerializedModel(Model):
    ...
    def from_directory(cls, model_path, manifest_path=None):
        warnings.warn(
            "SerializedModel is deprecated. Please use Model.load_from_manifest instead.",
            DeprecationWarning,
        )
...
class FrozenGraphModel(SerializedModel):
    ...
    def __init__(self, model_directory, manifest):
        ...
        for mandatory_key in self._mandatory_properties:
            ...
            ), "Mandatory property '{}' was not defined in json manifest.".format(
                mandatory_key
            )
        ...
        if len(layers_or_layer_names) > 0:
            if isinstance(layers_or_layer_names[0], str):
                ...
                self.layers = _layers_from_list_of_dicts(
                    self.__class__, layers_or_layer_names
                )
