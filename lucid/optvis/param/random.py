...
def rand_fft_image(shape, sd=None, decay_power=1):
    ...
    for _ in range(b):
        ...
        fh, fw = freqs.shape
        # spectrum_var = sd * tf.random_normal([2, ch, fh, fw], dtype="float32") # removing warning tf 1.15.5
        spectrum_var = sd * tf.random.normal([2, ch, fh, fw], dtype="float32") # removing warning tf 1.15.5
